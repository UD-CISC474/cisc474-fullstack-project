...
export class MarketComponent implements OnInit {
  ...
  ngOnInit(): void {
    const yesterday = this.getPreviousMarketDay();
    this.loadTickers(yesterday);
  ...
  getPreviousMarketDay(): string {
    // Determines the previous market day based on today
    const today = dayjs();
    if (today.day() === 1) {
      // If today is Monday, use Friday (3 days ago)
      return today.subtract(3, 'day').format('YYYY-MM-DD');
    } else if (today.day() === 0) {
      // If today is Sunday, use Friday (2 days ago)
  ...
  async loadTickers(date: string): Promise<void> {
    try {
      const tickerResponse = await this.marketService.getTickers(date);
      this.tickers = tickerResponse.results || [];
      this.selectedTicker = this.tickers.length ? this.tickers[0] : null;
      this.filterTickers();
    } catch (error) {
      console.error('Failed to load tickers', error);
      this.tickers = [];
  ...
  updateGraphPeriod(period: string): void {
    if (this.selectedTicker) {
      console.log(`Graph updated for period: ${period}`);
      // Placeholder: add logic to update/render graph based on selected period
    }
  }
